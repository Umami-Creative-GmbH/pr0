...
The desktop searches the Rust-owned SQLite projection through validated native commands. Schema version 8 combines offline organization, recovery and a persisted index of all visible downloaded and locally edited prompts. The prior version-6 layouts and both version-7 layouts (organization/recovery on main and search/recovery on this branch) upgrade without losing primary data or pending work; a compatible preview index is retained. A transaction drains changed prompt identities before committing primary data and the outbox. Projected organization names and usage metadata have separate update paths. Organization replay queues only final compact assignment changes: a name-only rename updates the name index without rewriting prompt metadata. Search uses field-specific compact trigram candidates, adaptive one/two-code-point postings, exact substring verification, and revision-bound pages. Body verification is limited to 64 rows and 4 MiB per batch; the SQLite page cache is 64 MiB, with no retained body cache.
...
- After merging main at `cab86e7`: all 109 native tests and ten desktop UI tests pass, including both version-7 migrations with pending work, offline organization/search updates, and unchanged draft preservation after selection moves. An index-write audit verifies zero prompt metadata writes for a tag rename and exactly one for a single assignment removal. Root tests, workspace typechecking, Ultracite, desktop production build and the optional WebView test build check pass. The two reported React Doctor loading-reset rules pass with `finally` cleanup. Standards and specification review findings were resolved.

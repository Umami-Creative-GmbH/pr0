...
- List and detail buttons share the native command and accessible status. UI completion is scoped to the mounted account generation. There is no new REST endpoint or OpenAPI shape: existing mutation/receipt contracts remain authoritative; typed native schemas and Tauri permissions are added together.
...
- `bun --env-file=apps/web/tests/device.env apps/web/tests/usage-runner.ts` with `PR0_TEST_BROWSER=msedge`: public REST conformance (3 tests), real native HTTPS approval/credential/process-restart journey, actual clipboard write, deliberately lost server response, receipt replay, delayed old usage, future correction, matching web/local counts and dates, archive/restore, and deletion before queued usage arrives. Passed.
- `bun test apps/web/tests/usage-ui.test.ts` with `PR0_TEST_BROWSER=msedge` and `PR0_DESKTOP_TEST_URL=http://127.0.0.1:14249`: production desktop React UI bridged to the real typed native service. Clipboard rejection retains the prompt, usage I/O failure reports successful copy separately, usage retry does not write the clipboard again, keyboard Copy updates one Recents row, reload retains it, and delayed completion cannot update a new UI generation. Passed, 6 assertions.
- Root `bun run test`: all workspace test tasks passed. Native full suite: 44 passed inside the sandbox; the existing credential subprocess test was denied access to Windows credentials and passed when rerun outside it. This covers all 45 native tests.

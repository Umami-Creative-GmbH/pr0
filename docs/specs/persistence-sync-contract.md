...
Use Better Auth database sessions with thirty-day expiry, `updateAge: 0`, cookie caching disabled and explicit validation/renewal on every protected request. Preserve renewed web cookies. Normal renewal extends database expiry without rotating the opaque token. Native credential replacement is needed on a new login, not every ordinary renewal.

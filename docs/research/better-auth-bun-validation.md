...
This follow-up investigates a provider-independent option for users who have only social login, while preserving the requirement to reauthenticate explicitly in the browser. The driving developer accepted the dedicated account/browser-bound email challenge with a five-minute TTL, three wrong attempts and a ten-minute marker in decision Q21; password users may instead explicitly re-enter their password. Generic OTP success alone is not sufficient. The account decision already permits recovery through the verified account email.
...
1. An authenticated browser explicitly requests reauthentication. Resolve the account and its already-verified current email from the server session, never from a submitted target email. Require a server-recorded browser session kind, apply CSRF/origin checks and verify that the account/session remain active. Set the session kind from the trusted issuance path. Header transport is not evidence of session kind: putting a desktop token into a Cookie header must not let it start or complete this challenge.
2. Create a cryptographically random challenge ID and an eight-digit cryptographically random email code, with a five-minute TTL and at most three failed verification attempts. Persist a purpose-specific row containing instance/account/browser-session IDs, the current email or email revision, a keyed digest of challenge ID plus code, expiry and attempt count. Keep the HMAC key server-only. Serialize issuance per account, invalidate the preceding outstanding reauth challenge when resending, and do not extend an existing challenge's TTL. Suggested separate send controls: one send per sixty seconds and three per fifteen minutes per account, with an additional deployment-defined IP limit. The five-minute TTL, three-failure budget and ten-minute marker are accepted Q21 decisions; the eight-digit code and send caps are suggested implementation defaults, not plugin behavior or additional accepted product decisions.

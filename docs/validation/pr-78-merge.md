...
The final refactor also extracts the name form, confirmation/review branches and state hook (`use-organization-manager.ts`). Both organization UI journeys still pass. React Doctor 0.9.14 continues to report the original 19/16 complexity on commit `51c98f7` (run `35597817213`), although a TypeScript AST inspection finds only five decision points in `OrganizationManager`, or six including callbacks. `apps/desktop/doctor.config.ts` suppresses only `react-doctor/no-high-complexity-react-function` in `src/organization-manager.tsx`; other rules and files remain checked. The config lives in the scanned workspace so its path is relative to the desktop project. Revisit this exception when upgrading the scanner.

Disabling React Doctor's optional worker pool (`REACT_DOCTOR_DISABLE_OXLINT_WORKER_POOL=1`) allows the local CLI to run under Bun without the IPC error. A fresh, complete scan of `organization-manager.tsx` with `--no-cache --no-score --no-supply-chain --no-parallel --yes --json` reports zero errors and warnings. The installed config loader confirms the desktop override is loaded. A full desktop scan still reports findings in other files; they are not covered by this exception. GitHub run `35598818996` continues to report the same warning even with scan caching disabled, so the ineffective workflow cache change was reverted. The remote warning remains unresolved; local validation does not establish that CI honors the override.

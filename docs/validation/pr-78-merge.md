...
Disabling React Doctor's optional worker pool (`REACT_DOCTOR_DISABLE_OXLINT_WORKER_POOL=1`) allows the local CLI to run under Bun without the IPC error. A fresh, complete scan of `organization-manager.tsx` with `--no-cache --no-score --no-supply-chain --no-parallel --yes --json` reports zero errors and warnings. The workflow disables scan caching to recompute findings after refactors and config changes. A full desktop scan still reports findings in other files; they are not covered by this exception.

...
The complete `test:account-deletion` runner also passed: deletion/rotation, seven restart boundaries, backup replay, browser cleanup, email changes, login methods, prompts, prompt deletion, organization cleanup, and account isolation. Legacy negative tests now expect validation or provenance rejection from supported device endpoints instead of treating those endpoints as absent. The desktop deletion regression observes the public receipt, unauthorized session, and invalidated device-code responses.

...
PostgreSQL atomically shares admission across processes: five signup attempts/hour/IP; three verification/recovery emails/hour/destination and twenty/hour/IP; ten failed sign-ins/15 minutes/account+IP and fifty/15 minutes/IP; sixty anonymous auth requests/minute/IP with ten-request bursts per ten seconds; 120 authenticated API requests/minute/account. Successful sign-ins refund their failed-login reservations. Email admission applies equally to unknown and known addresses. Throttling returns a stable code and `Retry-After` seconds.
...
```sh
docker build -t pr0-recovery-25 .
bun run --cwd apps/web test:accounts
bun run --cwd apps/web test:recovery
```
...
The recovery suite also uses public HTTP for reset/replay, cross-account session revocation, selected/all-other/self revocation, and shared email admission. Its isolated Compose override preloads a controlled JavaScript clock into the production server to prove 30-day inactivity and renewal. The normal production command has no clock override. Database fixtures represent a verified social-only account and expired tokens; SMTP outage, terminal delivery failure and outbox restart are observed through public requests and redacted operator diagnostics. See the [issue #25 validation report](recovery-validation.md) for results and review corrections.

## Recovery and session settings

The sign-in screen provides recovery through the account's verified email, even when registration is closed or the account currently has only a social login method. A request always returns the same generic acknowledgement for existing, unknown and unverified addresses. Verification and recovery share the three/hour/destination and twenty/hour/IP email limits and the bounded encrypted outbox. SMTP acceptance is not proof of inbox delivery. Users can retry through the recovery form; operators can inspect `mail-status` and `mail_delivery_failed` events (job ID, attempt number, terminal flag).

Recovery links expire one hour after issuance. Their bearer token is carried in a URL fragment, removed from browser history on arrival and retained only in the open form. Reloading that form requires opening the email link again. Never configure proxy or application tooling to capture request bodies or fragments containing passwords or tokens.

The public reset handler consumes the Better Auth recovery record, updates or creates its compatible credential account, revokes all existing sessions and invalidates outstanding recovery records in one PostgreSQL transaction. Better Auth still owns token issuance, password hashing and sign-in. The pinned schema/adapter integration is covered by public sign-in and concurrent replay acceptance tests. Invalid or expired reset attempts leave credentials and sessions unchanged; a successful reset requires a new sign-in.

Apply migration `004-session-issuance.sql` before serving this version. It preserves existing sessions and adds a server-owned authentication version. Sign-in captures that version before checking the password; PostgreSQL checks it again under the account lock when inserting the session. Reset advances the version in its transaction, so an overlapping old-password login cannot create a surviving session after reset. Missing issuance context fails closed. Future social/device issuance must establish the same server-owned context before authentication; never accept this version from a client.

Settings list separate public session identities, browser/device provenance, client description, sign-in/activity/expiry times and the current-session marker. Client descriptions are informational, not trusted device identity. A selected-session revocation affects only that account's selected identity; all-other revocation retains the initiating browser. Protected routes consult persisted sessions on each request, with cookie caching disabled, and renew active sessions for 30 days. Desktop presentation and retained-work reauthentication are implemented by the later desktop transition ticket.

This slice does not certify the full MVP's backup/restore, signed Windows distribution, actual OAuth provider login, prompt synchronization, capacity, or multi-browser accessibility release gates.

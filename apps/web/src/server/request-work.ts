...
export const withRequestWork = async <T>(
  operation: (claimOwner: (owner: string) => Promise<void>) => Promise<T>,
  signal?: AbortSignal
) => {
  await ensureDeletionRecovery();
  const sql = workDatabase();
  const id = crypto.randomUUID();
  const deadline = Date.now() + serviceLimit("REQUEST_QUEUE_MS", 5000);
  ...
  try {
    ...
    while (!admitted) {
      ...
      admitted = await sql.begin(async (tx) => {
        ...
        if (counts.active < serviceLimit("REQUEST_ACTIVE", 16)) {
        ...
        if (
          !existing.length &&
          counts.queued >= serviceLimit("REQUEST_QUEUE", 64)
        ) {
          throw new AccountFailureError("unavailable", 503, 5);
        }
        await tx`INSERT INTO request_work(id, expires_at) VALUES (${id}, ${new Date(deadline)}) ON CONFLICT DO NOTHING`;
        return false;
      });
      if (!admitted) {
        if (Date.now() >= deadline) {
    ...
    return await operation(async (owner) => {
      ...
      await sql.begin(async (tx) => {
        const [account] =
          await tx`SELECT id, suspended FROM "user" WHERE id=${owner} AND NOT deletion_pending`;
        if (!account) {
        ...
        if (counts.count >= serviceLimit("REQUEST_ACCOUNT", 4)) {
          throw new AccountFailureError("unavailable", 503, 5);
        }

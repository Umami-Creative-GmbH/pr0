...
test("records maximum-size text search including real debounce and rendering", async () => {
  ...
  try {
    await sql.begin(async (tx) => {
      ...
      await tx`INSERT INTO prompt(instance_id,account_id,id,title,description,content,revision,title_revision,description_revision,content_revision,created_at,modified_at)
        SELECT ${account.identity.instanceId}::uuid,${account.identity.accountId},gen_random_uuid(),'Prompt '||lpad(n::text,5,'0'),'',
        left(repeat(${phrase},100),10485 + CASE WHEN n<=7600 THEN 1 ELSE 0 END - 12),n,n,n,n,
      ...
      await tx`UPDATE library SET revision=10000,prompt_count=10000,text_bytes=(SELECT sum(octet_length(title)+octet_length(content)) FROM prompt WHERE account_id=${account.identity.accountId}) WHERE account_id=${account.identity.accountId}`;
    });
  } finally {
    await sql.close();
  }
  ...
  try {
    ...
    await page
      .getByRole("button", { name: "Prompt 00001", exact: true })
      .waitFor();
    for (const query of ["common", "abcd", "C++", "a"]) {
  ...
  await Bun.write(
    "docs/research/web-text-search-measurements.json",
    `${JSON.stringify(
      {
        ...
        fixture:
          "Synthetic repeated prose, 10000 prompts, exactly 100 MiB of canonical text; text fields only; local loopback production build.",
        coldMs,
        indexBytes: bytes,
        rest,
        rendering,
        releaseGate:
          "The final all-field 150 ms end-to-end gate on supported hardware remains required; these local text-only measurements do not certify release.",
      },
      null,
      2
    )}\n`

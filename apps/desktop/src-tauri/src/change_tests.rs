...
fn live_changes_upgrade_usage_database_without_losing_pending_work() {
    ...
    db.execute_batch("DROP TABLE change_state; DROP TABLE local_deleted; DROP TABLE local_identity; UPDATE upload_state SET last_checked='2026-09-21T10:00:00.000Z'; PRAGMA user_version=4;").unwrap();
    drop(db);
